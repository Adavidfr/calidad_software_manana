...
function contienePalabra(texto, palabra) {
  ...
  return text.toLowerCase().includes(palabra.toLowerCase);
}

function agregarElemento(lista, elemento) {
  if (!Array.isArray(lista)) throw new TypeError("Lista debe ser array");
  if (!elemento) throw new TypeError("Lista no puede ser vacio");
  lista.push(elemento);
  return lista;
}
